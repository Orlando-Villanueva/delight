...
<div {{ $attributes->merge(['class' => 'rounded-xl border border-[#D1D7E0] bg-white shadow-lg transition-colors dark:border-gray-700 dark:bg-gray-800']) }}>
    <div class="flex h-full flex-col gap-4 p-4">
        <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
                <p class="text-xs font-semibold uppercase tracking-wide text-blue-600 dark:text-blue-400">Next Milestone</p>
                <h2 class="mt-1 text-base font-semibold text-gray-900 dark:text-white">
                    {{ $milestone['display_name'] ?? ($latest ? 'Latest trophy' : 'First reading') }}
                </h2>
            </div>
            <a href="{{ route('achievements.index') }}"
                class="shrink-0 rounded-full bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                View shelf
            </a>
        </div>
        ...
        @if ($milestone)
            <div class="flex min-w-0 gap-3">
                <x-achievements.badge :icon="$milestone['icon'] ?? 'trophy'" :label="$milestone['display_name']" size="md" state="muted" />
                <div class="min-w-0">
                    <p class="font-semibold text-gray-900 dark:text-white">{{ $milestone['display_name'] }}</p>
                    <p class="mt-1 text-sm leading-5 text-gray-600 dark:text-gray-300">{{ $milestone['description'] }}</p>
                </div>
            </div>
            ...
            <div class="mt-auto">
                <div class="flex items-center justify-between gap-3 text-sm">
                    ...
                    <span class="text-xs font-semibold text-gray-500 dark:text-gray-400">{{ $milestone['current'] }}/{{ $milestone['target'] }}</span>
                </div>
                <div class="mt-2 h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                    <div class="h-full rounded-full bg-blue-600 dark:bg-blue-400" style="width: {{ $milestone['progress_percent'] }}%"></div>
                </div>
            </div>

            @if ($latest)
                <p class="border-t border-gray-100 pt-3 text-xs font-medium text-gray-500 dark:border-gray-700 dark:text-gray-400">
        ...
        @else
            <div class="flex min-w-0 gap-3">
                ...
                <div class="min-w-0">
                    <p class="font-semibold text-gray-900 dark:text-white">First reading</p>
                    <p class="mt-1 text-sm leading-5 text-gray-600 dark:text-gray-300">
                        Log your first Bible reading.
                    </p>
                </div>
            </div>
