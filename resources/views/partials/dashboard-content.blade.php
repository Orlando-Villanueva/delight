...
<div class="space-y-6 lg:space-y-4 xl:space-y-6 lg:pb-0" id="dashboard-main-content-wrapper"
    hx-trigger="readingLogAdded from:body" hx-get="{{ route('dashboard') }}" hx-target="#main-content" hx-swap="outerHTML"
    hx-select="#main-content" hx-disinherit="hx-select">
    @php
    ...
    <div class="grid grid-cols-1 xl:grid-cols-4 gap-6 lg:gap-4 xl:gap-6">
        ...
        <div class="xl:col-span-3 space-y-4 xl:space-y-6">
            ...
            <div
                ...
                <div class="sm:col-span-1 lg:col-span-1 xl:col-span-1">
                    <x-ui.weekly-journey-card :currentProgress="$weeklyJourneyCard['currentProgress']" :days="$weeklyJourneyCard['days']" :weekRangeText="$weeklyJourneyCard['weekRangeText']" :weeklyTarget="$weeklyJourneyCard['weeklyTarget']"
                        :ctaEnabled="$weeklyJourneyCard['ctaEnabled']" :ctaVisible="$weeklyJourneyCard['ctaVisible']" :status="$weeklyJourneyCard['status']" :journeyAltText="$weeklyJourneyCard['journeyAltText']" class="h-full" />
                </div>
                ...
                <div class="col-span-1 sm:col-span-1 md:col-span-1 lg:col-span-1 xl:col-span-2 2xl:col-span-1">
                    <x-ui.summary-stats :daysRead="$stats['reading_summary']['total_reading_days']" :totalChapters="$stats['reading_summary']['total_readings']" :bibleProgress="$stats['book_progress']['overall_progress_percent']" :averageChaptersPerDay="$stats['reading_summary']['average_chapters_per_day']"
                        class="h-full" />
                </div>
        ...
        <div class="hidden xl:block xl:col-span-1 space-y-6 xl:space-y-6" id="dashboard-sidebar"
            ...
            <div class="xl:max-w-none">
                <x-bible.calendar-heatmap :calendar="$calendarData['calendar']" :monthName="$calendarData['monthName']" :thisMonthReadings="$calendarData['thisMonthReadings']" :thisMonthChapters="$calendarData['thisMonthChapters']"
